...
class LSTM(Module):
    def __init__(self, input_size, hidden_size, num_layers=1, bias=True, device=None, dtype="float32"):
        ...
        ### BEGIN YOUR SOLUTION
        self.num_layers = num_layers
        lstm_cells = [LSTMCell(input_size, hidden_size, bias, device=device, dtype=dtype)]
        for i in range(num_layers-1):
            lstm_cells.append(LSTMCell(hidden_size, hidden_size, bias, device = device, dtype=dtype))
