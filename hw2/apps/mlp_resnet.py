...
def ResidualBlock(dim, hidden_dim, norm=nn.BatchNorm1d, drop_prob=0.1):
    ### BEGIN YOUR SOLUTION
    modules = nn.Sequential(nn.Linear(dim, hidden_dim),
                           norm(hidden_dim),
                           nn.ReLU(),
...
def MLPResNet(dim, hidden_dim=100, num_blocks=3, num_classes=10, norm=nn.BatchNorm1d, drop_prob=0.1):
    ### BEGIN YOUR SOLUTION
    modules = [nn.Linear(dim, hidden_dim),
               nn.ReLU()]
...
def train_mnist(batch_size=100, epochs=10, optimizer=ndl.optim.Adam,
    ...
    np.random.seed(4)
    ### BEGIN YOUR SOLUTION
    train_data = ndl.data.MNISTDataset(data_dir+"/train-images-idx3-ubyte.gz",
                                       data_dir+"/train-labels-idx1-ubyte.gz")
    test_data = ndl.data.MNISTDataset(data_dir + "/t10k-images-idx3-ubyte.gz",
    ...
    for _ in range(epochs):
        train_cc, train_loss = epoch(train_loader, model, opt)
        test_cc, test_loss = epoch(test_loader, model)
    return (train_cc, train_loss, test_cc, test_loss)
    ### END YOUR SOLUTION
